...
import {createContext, useContextSelector as useContextSelectorExternal} from 'use-context-selector';
...
const Provider = ({onRender, useSelector, selector}) => {
  ...
  return (
    <Context.Provider value={state}>
      <Component setState={setState} onRender={onRender} useSelector={useSelector} selector={selector} />
    </Context.Provider>
  );
};
...
it("doesn't rerender", () => {
  ...
  const {getByTestId} = render(
    <Provider onRender={onRender} useSelector={useContextSelector} selector={({title}) => ({title})} />
  );
  expect(onRender).toBeCalledTimes(1);
  fireEvent.click(getByTestId('toggle'));
  expect(onRender).toBeCalledTimes(1);
